    ...
    <div className='shopping-cart-page'>
      {cart.length === 0 ? (
        <div id="shopping-cart-list-empty">
          <img src="https://cdni.iconscout.com/illustration/premium/thumb/empty-cart-2130356-1800917.png" alt="" width='400px' height='400px'/> <br />
    ...
    >
      ...
      <td style={{ textAlign: 'center'}}>
        <div className='cart-border'>
          <button onClick={() => reduceQuantity(product.id)} className='quantity-button-decrease'>-</button>
          {product.count}
          <button onClick={() => increaseQuantity(product.id)} className='quantity-button-increase'>+</button>
        </div>
      </td>
      <td className='table-sell-style'>{(product.count * parseFloat(product.price)).toFixed(2)}</td>
      <td className='table-sell-style'>
        <button onClick={() => removeFromCart(product.id)} className='action-button-style'><i className="fa fa-trash" aria-hidden="true"></i></button>
      </td>
    </tr>
      ...
      <div className='check-point-proceed'>
        ...
        <p className='' style={{borderBottom: '1px solid black', marginTop: '3em'}} >Total <span style={{float: 'right'}}>&cent;{calculateTotalAmount(cart)}</span> </p> 
          <div className='proceed-to-checkpoint-button' style={{ display: 'flex', justifyContent: 'center' }}>
            <button style={{fontWeight: 'bolder', width: '100%', marginTop: '5em', padding: '15px 10px', borderRadius: '20px',border: 'none', backgroundColor: 'rgb(89, 172, 89)', cursor: 'pointer'}} onClick={() => handleCheckpointClick()}>Proceed To Checkpoint</button>
          </div>
          </div>
      </div>
      </div>
