...
const handleSubmitOrder = () => {
  ...
  return (
    <div>
      <h2 style={{ textAlign: 'center', marginTop: '4em' }}>Selected Products</h2>
      {productIds ? (
        <div>
        <table className="check-point-table" style={{ margin: '0 auto' }}>
          ...
          <tbody>
            {productIds.map((productId) => {
              const product = cart ? cart.find((item) => item.id === productId) : null;
              return (
                <tr key={productId}>
        ...
        <div style={{width: '50%', padding: '3rem'}}>
          <button onClick={() => handleSubmitOrder()} style={{ cursor: 'pointer',float: 'right', padding: '0.5em'}}>Place Order</button>
        </div>
    </div>
      ) : (
